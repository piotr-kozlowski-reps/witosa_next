...
import { TArtisticGroupWithImages } from '@/types';
import { ArtisticGroup } from '@prisma/client';
import { create } from 'zustand';

// const artisticGroups: ArtisticGroup[] = [
//   {
//     id: '2',
...
interface ArtisticGroupsStore {
  artisticGroups: TArtisticGroupWithImages[];
  getArtisticGroup: (title: string) => TArtisticGroupWithImages | undefined;
  fetchArtisticGroups: () => Promise<void>;
}
...
export const useArtisticGroupsStore = create<ArtisticGroupsStore>()(
  (set, get) => ({
    ...
    fetchArtisticGroups: async () => {
      try {
        const result = await getAllArtisticGroups();
        ...
        if (
          ...
          set({ artisticGroups: result.response });
        }
      } catch (error) {
        throw error;
      }
